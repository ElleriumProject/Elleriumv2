...
const string strMessageMagic = "Ellerium Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if(nHeight > 0 && nHeight <= 20000) {
        nSubsidy = 3 * COIN;
	} else if (nHeight > 20000 && nHeight <= 1000000) {
		nSubsidy = 2 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if(nHeight > 0 && nHeight <= 20000) {
		...
		if (mNodeCoins == 0) {
			...
			LogPrintf("[LIBRA] Adjusting Libra at height %d with %d masternodes (%d % locked Ellerium) and %d Ellerium supply at %ld\n", nHeight, nMasternodeCount, lockedCoinValue*100, nMoneySupply, GetTime());
